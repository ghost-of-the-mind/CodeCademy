////////////// For adding new card digits, takes in a number and splits it into individual digits

const addNewCard = (digits) => {
   
  let cardDigits = Array.from(digits.toString()).map(Number);

  return cardDigits
};

console.log(addNewCard(34544567543))


/* CARDS To TEST */



    // All valid credit card numbers
    const valid1 = [4, 5, 3, 9, 6, 7, 7, 9, 0, 8, 0, 1, 6, 8, 0, 8];
    const valid2 = [5, 5, 3, 5, 7, 6, 6, 7, 6, 8, 7, 5, 1, 4, 3, 9];
    const valid3 = [3, 7, 1, 6, 1, 2, 0, 1, 9, 9, 8, 5, 2, 3, 6];
    const valid4 = [6, 0, 1, 1, 1, 4, 4, 3, 4, 0, 6, 8, 2, 9, 0, 5];
    const valid5 = [4, 5, 3, 9, 4, 0, 4, 9, 6, 7, 8, 6, 9, 6, 6, 6];
    // All invalid credit card numbers
    const invalid1 = [4, 5, 3, 2, 7, 7, 8, 7, 7, 1, 0, 9, 1, 7, 9, 5];
    const invalid2 = [5, 7, 9, 5, 5, 9, 3, 3, 9, 2, 1, 3, 4, 6, 4, 3];
    const invalid3 = [3, 7, 5, 7, 9, 6, 0, 8, 4, 4, 5, 9, 9, 1, 4];
    const invalid4 = [6, 0, 1, 1, 1, 2, 7, 9, 6, 1, 7, 7, 7, 9, 3, 5];
    const invalid5 = [5, 3, 8, 2, 0, 1, 9, 7, 7, 2, 8, 8, 3, 8, 5, 4];
    // Can be either valid or invalid
    const mystery1 = [3, 4, 4, 8, 0, 1, 9, 6, 8, 3, 0, 5, 4, 1, 4];
    const mystery2 = [5, 4, 6, 6, 1, 0, 0, 8, 6, 1, 6, 2, 0, 2, 3, 9];
    const mystery3 = [6, 0, 1, 1, 3, 7, 7, 0, 2, 0, 9, 6, 2, 6, 5, 6, 2, 0, 3];
    const mystery4 = [4, 9, 2, 9, 8, 7, 7, 1, 6, 9, 2, 1, 7, 0, 9, 3];
    const mystery5 = [4, 9, 1, 3, 5, 4, 0, 4, 6, 3, 0, 7, 2, 5, 2, 3];
    // An array of all the arrays above
    const batch = [valid1, valid2, valid3, valid4, valid5, invalid1, invalid2, invalid3, invalid4, invalid5, mystery1, mystery2, mystery3, mystery4, mystery5];
    /* FOR TESTING PURPOSES */

////////////// Checks if a card's digits are valid by using the Luhn algorithm

const validateCred = arr => {

  let digits = [];

  if (arr.length % 2 == 0) {
    for (let i = arr.length-1; i >= 0; i--) {
      if (i % 2 == 0) {
        let preDigitOne = arr[i] * 2;
          if (preDigitOne > 9) {
            let digit = preDigitOne - 9;
            digits.push(digit);
          } else {
            digits.push(preDigitOne);
          }
      } else {
        let preDigitTwo = arr[i];
        digits.push(preDigitTwo);
      } 
    }
  } else {
    for (let i = arr.length-1; i >= 0; i--) {
      if (i % 2 !== 0) {
        let preDigitOne = arr[i] * 2;
          if (preDigitOne > 9) {
            let digit = preDigitOne - 9;
            digits.push(digit);
          } else {
            digits.push(preDigitOne);
          }
      } else {
        let preDigitTwo = arr[i];
        digits.push(preDigitTwo);
      } 
    }
  }
  
  const sum = digits.reduce((a, b) => {return a + b})

  if (sum % 10 == 0) {
    return `This card is valid!`
  } else {
    return `This card is not valid!`
  }
};

////////////// Validate or invalidate all of the cards
/* for (i = 0; i < batch.length; i++) {
  console.log(validateCred(batch[i]));
}; */



////////////// Makes a list of all of the invalid cards

let invalidCards = [];

const findInvalidCards = arr => {

  let result = validateCred(arr);

  if (result == 'This card is not valid!') {
    invalidCards.push(arr);
  } 
  return invalidCards
};

/* for (i = 0; i < batch.length; i++) {
  findInvalidCards(batch[i]);
}; */

////////////// Prints a list of all the invalid cards

/* console.log(invalidCards); */

////////////// For finding the companies that issues the invalid cards

let companies = new Set();

const idInvalidCardCompanies = arr => {

  let result = validateCred(arr);

  if (result == 'This card is not valid!') {
    switch (arr[0]) {
      case 3:
        companies.add("Amex")
        //console.log("Company added to list");
        break;
      case 4:
        companies.add("Visa")
        //console.log("Company added to list");
        break;
      case 5:
        companies.add("Mastercard")
        //console.log("Company added to list");
        break;
      case 6:
        companies.add("Discover")
        //console.log("Company added to list");
        break;
      default:
        companies.add("Unknown Company");
    }
  }
};

for (i = 0; i < batch.length; i++) {
  idInvalidCardCompanies(batch[i]);
};

let compList = Array.from(companies)

////////////// Prints a list of all the companies (without duplicates) that have setn us an invalid card

/* console.log(compList); */
